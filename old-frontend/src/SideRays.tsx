// src/SideRays.tsx
// React Bits SideRays (TS wrap). Animated god-rays emitted from a corner
// of the container, rendered via ogl in a WebGL canvas. Pure background
// effect — sets `pointer-events: none` on the container so it never
// intercepts clicks.
//
// Mount inside a `position: relative` (or `absolute/fixed`) wrapper so the
// canvas can size itself to the parent. The container size IS the canvas
// size — give the parent explicit width/height (or use `inset: 0`).
