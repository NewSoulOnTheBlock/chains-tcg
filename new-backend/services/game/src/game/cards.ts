// ─────────────────────────────────────────────────────────────────────────────
// VENDORED COPY of <repo>/src/cards.ts — DO NOT EDIT BY HAND.
//
// Why vendored and not a path dependency: every service Dockerfile is built
// with `new-backend/` as its context, so nothing above `new-backend/` (i.e. the
// frontend's `src/`) can be COPY'd into the image. A `file:../../..` dependency
// would also drag the Vite/React tsconfig into a Node-only service.
//
// Re-sync with:  cp ../../../src/cards.ts src/game/cards.ts
//
// Local type-only fix (behaviour identical, required by `noUncheckedIndexedAccess`):
//   `CARDS[id].name` -> `CARDS[id]?.name ?? id` in validateDeck's copies message.
//   (`id` is already known to be a real card there — unknown ids `continue` above.)
// (then re-apply this header). The only functions this service uses are
// `validateDeck`, `CARDS`, `DECK_SIZE` and `derivePrimaryColor`; the rest is
// carried verbatim so the file stays a byte-for-byte diff against upstream.
// ─────────────────────────────────────────────────────────────────────────────
// src/cards.ts
...
export function validateDeck(cards: string[], opts?: { requireSize?: boolean }): DeckValidation {
  ...
  for (const [id, n] of Object.entries(counts)) {
    if (!isBasicNode(id) && n > MAX_COPIES_NONBASIC) {
      issues.push({
        code: 'copies',
        message: `Too many copies of ${CARDS[id]?.name ?? id} (${n}/${MAX_COPIES_NONBASIC}).`,
      });
    }
  }
